angular
  .module("Agenvida.controllerReporte", ["chart.js"])
  .controller("controllerReporte", function(
    $scope,
    $state,
    $http,
    $window,
    ...
    $scope.hoy = new Date();
    $scope.vinculaciones = [
      { id: 5, nombre: "Proposito Particular" },
      { id: 1, nombre: "Dios" },
      { id: 2, nombre: "Conmigo" },
    ...
    $scope.actualizarReporte = function() {
      $scope.propositos_mes = $filter("filter")($scope.propositos, {
        mes_ano:
          $scope.anos.selectedOption + "-" + $scope.meses.selectedOption + "-"
      }); //traigo los propositos del mes
      ...
      for (var i = 0; i < $scope.propositos_mes.length; i++) {
        ...
        if ($scope.propositos_mes[i].vinculacion == 1) {
          $scope.propositos_mes[i].vinculacion_nombre = "Dios";
        }
        if ($scope.propositos_mes[i].vinculacion == 2) {
          $scope.propositos_mes[i].vinculacion_nombre = "Conmigo";
        }
        if ($scope.propositos_mes[i].vinculacion == 3) {
          $scope.propositos_mes[i].vinculacion_nombre = "Los Demas";
        }
        if ($scope.propositos_mes[i].vinculacion == 4) {
          $scope.propositos_mes[i].vinculacion_nombre = "Naturaleza";
        }
        if ($scope.propositos_mes[i].vinculacion == 5) {
          $scope.propositos_mes[i].vinculacion_nombre = "Proposito Particular";
        }
        ...
        for (var j = 0; j < $scope.propositos_mes[i].marcaciones.length; j++) {
          ...
          if ($scope.marcacion.cumplimiento == 1) {
            $scope.propositos_mes[i].positivo =
              $scope.propositos_mes[i].positivo + 1;
          } else if ($scope.marcacion.cumplimiento == 2) {
            $scope.propositos_mes[i].negativo =
              $scope.propositos_mes[i].negativo + 1;
          } else {
            $scope.propositos_mes[i].neutro =
              $scope.propositos_mes[i].neutro + 1;
          }
        } //endFor
      ...
      $scope.propositos_dios = $filter("filter")($scope.propositos_mes, {
        vinculacion: 1
      });
      $scope.propositos_conmigo = $filter("filter")($scope.propositos_mes, {
        vinculacion: 2
      });
      $scope.propositos_losDemas = $filter("filter")($scope.propositos_mes, {
        vinculacion: 3
      });
      $scope.propositos_naturaleza = $filter("filter")($scope.propositos_mes, {
        vinculacion: 4
      });
      $scope.propositos_pparticular = $filter("filter")($scope.propositos_mes, {
        vinculacion: 5
      });
      ...
      for (var j = 0; j < 6; j++) {
        $scope.propositos_aux = $filter("filter")($scope.propositos_mes, {
          vinculacion: j
        });
        if (j == 0) {
        ...
        for (var i = 1; i < $scope.propositos_aux.length; i++) {
          if (
            $scope.maxProposito[j].positivo < $scope.propositos_aux[i].positivo
          ) {
            $scope.maxProposito[j] = $scope.propositos_aux[i];
          }

          if (
            $scope.minProposito[j].negativo < $scope.propositos_aux[i].negativo
          ) {
            $scope.minProposito[j] = $scope.propositos_aux[i];
          }

          $scope.totalActual =
            $scope.blancoProposito[j].negativo +
            $scope.blancoProposito[j].positivo +
            $scope.blancoProposito[j].neutro;
          $scope.totalNuevo =
            $scope.propositos_aux[i].negativo +
            $scope.propositos_aux[i].positivo +
            $scope.propositos_aux[i].neutro;
