angular
  ...
  .controller("controllerPerfil", function(
    ...
    $scope.editar = function(campo) {
      ...
      if (campo == "nacimiento") {
        ...
        if ($scope.perfil.nacimiento != null) {
          ...
          $scope.nacimiento = new Date(
            $scope.nacimiento[0],
            $scope.nacimiento[1] - 1,
            $scope.nacimiento[2]
          );
        } else {
      ...
      if (campo == "reafirmar" || campo == "adquirir" || campo == "liberar") {
        $scope.template =
          '<textarea ng-model="perfil.' + campo + '" rows="4"></textarea>';
      } else {
        $scope.template =
          '<input type="' +
          $scope.inputType +
        ...
        buttons: [
...

          {
            ...
            onTap: function(e) {
              if (campo == "nacimiento") {
                $scope.nacimiento =
                  $scope.perfil.nacimiento.getFullYear() +
                  "-" +
                  ($scope.perfil.nacimiento.getMonth() + 1) +
                  "-" +
                  $scope.perfil.nacimiento.getDate();
                // $scope.perfil.nacimiento = $scope.nacimiento ;
                // console.log($scope.perfil.nacimiento);
                ...
                $http
                  .put($rootScope.domain + "userProfile/", $scope.perfil)
                  .then(function(result) {
                    $scope.perfil = result.data;
                  });
              }
    ...
    $scope.editarUser = function(campo) {
      ...
      $scope.template =
        '<input type="' + "text" + '" ng-model="user.' + campo + '">';
